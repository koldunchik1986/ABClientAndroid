...
namespace ANClient.ExtMap
{
    internal static class Map
    {
        ...
        internal static SortedDictionary<string, AncCell> AncCells { get; private set; }
        ...
        static Map()
        {
            ...
            AncCells = new SortedDictionary<string, AncCell>();
        ...
        private static void LoadAncMap()
        {
            var pathCellsXml = Path.Combine(Application.StartupPath, "ancells.xml");
            ...
            try
            {
                ...
                var cellsNodeList = xmlDocument.GetElementsByTagName(AppConsts.TagAncMapCell);
                foreach (XmlNode cellNode in cellsNodeList)
                {
                    if (cellNode.Attributes == null)
                        continue;

                    var ancCell = new AncCell();
                    if (cellNode.Attributes[AppConsts.AttrAncMapRegNum] == null)
                        continue;

                    ancCell.RegNum = cellNode.Attributes[AppConsts.AttrAncMapRegNum].Value;
                    if (cellNode.Attributes[AppConsts.AttrAncMapLabel] != null)
                    {
                        ancCell.Label = cellNode.Attributes[AppConsts.AttrAncMapLabel].Value;
                        if (Cells.ContainsKey(ancCell.RegNum))
                        {
                            if (
                                !ancCell.Label.Equals(Cells[ancCell.RegNum].Tooltip,
                                    StringComparison.CurrentCultureIgnoreCase))
                            {
                                //sb.Append(
                                //    $"Клетка [{ancCell.RegNum}] помечена как [{Cells[ancCell.RegNum].Tooltip}], должно быть [{ancCell.Label}]");
                                //sb.AppendLine();
                            }
                        }
                        else
                        {
                            //sb.Append($"Клетка [{ancCell.RegNum}] отсутствует");
                            //sb.AppendLine();
                        }
                    }

                    var cost = 0;
                    if (cellNode.Attributes[AppConsts.AttrAncMapCost] != null)
                    {
                        if (!int.TryParse(cellNode.Attributes[AppConsts.AttrAncMapCost].Value, out cost))
                        {
                            cost = 0;
                        }
                    }
                    ...
                    if (ancCell.Cost == 0)
                    {
                        if (Cells.ContainsKey(ancCell.RegNum))
                        {
                            var jcost = Cells[ancCell.RegNum].Cost;
                            if (jcost == 21)
                                ancCell.Cost = 30;
                            else
                            {
                    ...
                    if (cellNode.Attributes[AppConsts.AttrAncMapVisited] != null)
                    {
                        if (!DateTime.TryParse(cellNode.Attributes[AppConsts.AttrAncMapVisited].Value, AppVars.EnUsCulture, DateTimeStyles.None, out visited))
                        {
                            visited = DateTime.MinValue;
                        }
                    }

                    ancCell.Visited = visited;

                    var verified = DateTime.MinValue;
                    if (cellNode.Attributes[AppConsts.AttrAncMapVerified] != null)
                    {
                        if (!DateTime.TryParse(cellNode.Attributes[AppConsts.AttrAncMapVerified].Value, AppVars.EnUsCulture, DateTimeStyles.None, out verified))
                        {
                            verified = DateTime.MinValue;
                        }
                    }

                    ancCell.Verified = verified;

                    AncCells.Add(ancCell.RegNum, ancCell);
                }
            }
            catch
            {
        ...
        private static void CompareMaps()
        {
            ...
            {
                if (AncCells.ContainsKey(cell.Value.CellNumber))
                    continue;
            ...
            foreach (var ancCell in AncCells)
            {
                if (Cells.ContainsKey(ancCell.Value.RegNum))
                    continue;

                var cell = new Cell
                {
                    CellNumber = ancCell.Value.RegNum,
                    Name = ancCell.Value.Label
                };
        ...
        {
            var pathTeleportsXml = Path.Combine(Application.StartupPath, "anteleports.xml");
        ...
        public static void SaveAncMap()
        {
            ...
            foreach (var ancCell in AncCells)
            {
                xw.WriteStartElement(AppConsts.TagAncMapCell);

                xw.WriteStartAttribute(AppConsts.AttrAncMapRegNum);
                xw.WriteString(ancCell.Value.RegNum);
                xw.WriteEndAttribute();

                xw.WriteStartAttribute(AppConsts.AttrAncMapLabel);
                xw.WriteString(ancCell.Value.Label);
                xw.WriteEndAttribute();

                xw.WriteStartAttribute(AppConsts.AttrAncMapCost);
                xw.WriteValue(ancCell.Value.Cost);
                xw.WriteEndAttribute();

                xw.WriteStartAttribute(AppConsts.AttrAncMapVisited);
                xw.WriteString(ancCell.Value.Visited.ToString(AppVars.EnUsCulture));
                xw.WriteEndAttribute();

                xw.WriteStartAttribute(AppConsts.AttrAncMapVerified);
                xw.WriteString(ancCell.Value.Verified.ToString(AppVars.EnUsCulture));
                xw.WriteEndAttribute();
            ...
            var fileStream = new FileStream("ancells_new.xml", FileMode.Create);
            ...
            try
            {
                File.Delete("ancells.xml");
                File.Move("ancells_new.xml", "ancells.xml");
            }
            catch (IOException)
            {
            }
        ...
        internal static string CellDivText(int x, int y, int scale, string link, bool showmove, bool isframe)
        {
            ...
            AncCell anccell;
            if (AncCells.TryGetValue(regnum, out anccell))
            {
                var span = DateTime.Now.Subtract(AppVars.Profile.ServDiff).Subtract(anccell.Visited);
                if (span.TotalDays < 1.0)
                {
                    var visitedcolor = HexColorVisited(span.TotalHours);
                    sb.Append($"<br><span style=\"color:{visitedcolor}\">{anccell.Visited.Hour:D2}:{anccell.Visited.Minute:D2}</span>");
                }
            }
