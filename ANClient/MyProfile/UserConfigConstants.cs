namespace ANClient.MyProfile
{
    ...
    {
        private const string ConstCommentOne = " Profile for ANClient, the fast client for Neverlands (https://github.com/wmlabtx/anclient/wiki/) ";
        ...
        private const string ConstAutoAnswerDefault =
            /*
            "Это автоответ ANClient. Не ждали?" + AppConsts.Br +
            "Я все понимаю, но это автоответ ANClient" + AppConsts.Br +
            "Хватит! Автоответу ANClient это не нравится" + AppConsts.Br +
            "Это автоответ ANClient, не старайся мне что-то объяснить" + AppConsts.Br +
            "Хозяин отошел, но я ему это передам. Это автоответ ANClient" + AppConsts.Br +
            "Автоответ ANClient. Я ничего не читаю, что ты мне пишешь" + AppConsts.Br +
            "Серьезно? Это автоответ ANClient" + AppConsts.Br +
            "Что-то? Повтори. Автоответ ANClient плохо тебя понимает" + AppConsts.Br +
            "Перезагрузись, от тебя закорючки идут. Автоответ ANClient" + AppConsts.Br +
            "Автоответ ANClient советует тебе помолчать" + AppConsts.Br +
            "Это автоответ ANClient. Оставьте сообщение после длинного гудка" + AppConsts.Br +
            "Ты расстраиваешь автоответ ANClient своими глупостями" + AppConsts.Br +
            "Автоответ ANClient ненавидит спаммеров... Где моя нападалка?..." + AppConsts.Br +
            "А в рыло? Автоответ ANClient не понимает шуток" + AppConsts.Br +
            "Ну даешь! Нравится говорить с автоответом ANClient?" + AppConsts.Br +
            "Давай, давай. Ты только заводишь автоответ ANClient" + AppConsts.Br +
            "Автоответ ANClient думает, что это бред" + AppConsts.Br +
            "Как вы все меня утомили! Это автоответ ANClient" + AppConsts.Br +
            "Пиши еще. Автоответ ANClient питается твоими словами" + AppConsts.Br +
            "Я так и передам МСу. Это автоответ ANClient" + AppConsts.Br +
            "Не мешай автоответу ANClient медитировать" + AppConsts.Br +
            "Автоответ ANClient думает, просит не мешать" + AppConsts.Br +
            "Что-что? Пиши медленней, автоответ ANClient не успевает за тобой" + AppConsts.Br +
            "Я хоть и бот, но обидчивый. Автоответ ANClient может и боевую влепить" + AppConsts.Br +
            "Ты говоришь с автоответом ANClient, но не расстраивайся. Хозяин ненамного умнее меня" + AppConsts.Br +
            "Ты даже автоответ ANClient сумел разозлить!" + AppConsts.Br +
            "Что за ерунду ты мне пишешь? Автоответ ANClient ничего не понимает!" + AppConsts.Br +
