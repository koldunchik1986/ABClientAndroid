    ...
    internal sealed partial class FormMain
    {
        ...
        internal static void MakeVisit(int x, int y)
        {
            ...
            if (Map.Location.TryGetValue(coor, out position))
            {
                var regnum = Map.Location[coor].RegNum;
                AncCell anccell;
                if (Map.AncCells.TryGetValue(regnum, out anccell))
                {
                    anccell.Visited = (AppVars.Profile.ServDiff == TimeSpan.MinValue) ? DateTime.Now : DateTime.Now.Subtract(AppVars.Profile.ServDiff); ;
                }
            }
        }
