...
## Цель
- [x] Перенести 1:1 поведение ПК-кнопки `buttonFury` в Android как чекбокс во вкладке `Авто-бой -> Ротация` (по группам).
...
- [x] Сохранить совместимость с текущим пайплайном `MainPhp`/`LezFight`/настроек профиля.
...
5. [x] Реализовать боевую часть в `LezFight`:
   - разрешить scroll-hit по активной группе ротации с `DoFury`;
   - детект факта удара свитком по боевым логам;
   - после первого подтвержденного scroll-hit выключать runtime `DoFury`, писать статус в чат.
6. [x] Прогнать компиляцию:
   - `./gradlew :app:compileDebugJavaWithJavac`.

## Критерии готовности
- [x] В `Авто-бой -> Ротация` есть чекбокс режима свитка осады, сохраняется в профиль.
- [x] При включённом режиме и ненадетом свитке клиент сам доходит до `wear`-запроса и надевает нужный свиток.
- [x] Первый удар в бою использует `Снежок`/`Свиток Удар Ярости` по выбранной группе ротации (без отдельного фильтра boss).
- [-] После первого успешного scroll-hit режим свитка и автобой отключаются автоматически. (устарело: по текущей логике выключается только runtime Fury, автобой не гасим)
- [x] Сборка проходит без регрессий.
...
- [x] В `LezFight` добавлен C#-паритет:
  - scroll-hit разрешается при `DoFury` в выбранной группе ротации,
  - по факту первого scroll-hit runtime-режим Fury выключается автоматически (без отключения AutoBoi).
