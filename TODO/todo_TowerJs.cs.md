# План портирования TowerJs.cs

Файл `TowerJs.cs` является частью `PostFilter` и предназначен для обработки JavaScript-файла, связанного с Башней Смерти (вероятно, `/js/tower.js`).

## Функциональность в C#

Фильтр выполняет одно простое действие: он берет содержимое файла-ресурса `json2.js` и добавляет его в самое начало скачиваемого скрипта.

**Назначение:**

Эта логика полностью идентична `CastleJs.cs`, `OutpostJs.cs` и `TarenaJs.cs`. Библиотека `json2.js` является полифилом, который добавляет поддержку `JSON.parse()` и `JSON.stringify()` в старые браузеры (Internet Explorer), где эта функциональность отсутствует. Это было необходимо, так как скрипт Башни Смерти, по-видимому, использует функции для работы с JSON.
...
1.  **Создать `TowerJs.java`**: В пакете `ru.neverlands.abclient.postfilter`.
...
3.  **Интегрировать в `Filter.java`**: Добавить вызов `TowerJs.process(array)` в соответствующий блок.
