...
- [x] Финальная проверка всего потока на runtime-логах закрыта (серия логов с капчей/без капчи, включая повторные циклы).
...
- [x] Протестировать на нескольких runtime-логах с капчей и без.
...
- [x] При включении `Авто-Рыбалка` снасти стабильно проверяются и надеваются в оба слота.
- [x] Приманка выбирается строго по настройкам и доступности.
- [x] При капче диалог появляется корректно, submit проходит, цикл продолжается.
- [x] Отчет ловли стабильно попадает в UI/чат и корректно считает NV/массу.
- [x] Нет циклических перезаходов и подвисаний в `main.php`.

## Следующий фокус

- [x] Взять 2-3 свежих `logcat_runtime_*.txt` по рыбалке (с капчей/без) и закрыть пункты валидации Этапа 4 и 5.
