...
- [x] При `DoProxy=false` трафик стабильно идет через локальный proxy (direct egress).
- [x] При `DoProxy=true` и `ProxyAddress` трафик идет через локальный proxy + upstream tunnel.
- [x] Настройки прокси полностью читаются/пишутся в `.profile` совместимо с C# форматом.
- [x] Логи позволяют точно восстановить путь каждого сбоя.
