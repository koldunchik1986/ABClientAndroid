...
## Установка profile.reg на устройстве

- В окнах `Требуется лицензия` и `Запрос лицензии создан` кнопка `Прикрепить` открывает системный выбор файла Android.
- Пользователь выбирает полученный `profile.reg`; приложение проверяет префикс `ANREG1:`/`ANREG2:` и копирует файл в путь, показанный в строке `Куда положить profile.reg`.
- На экране входа после успешного копирования приложение автоматически повторяет проверку лицензии и продолжает вход, если `profile.reg` подходит текущему профилю.
- На экране профиля после успешного копирования приложение сразу валидирует файл и показывает результат.
