...
- [x] По логам ПК `ANClient/bin/Debug/Logs/Critical/20260522_11_50_*` подтверждён феномен с `NeverTimer`: при активном таймере (`SetNeverTimer 156000`) сетевой контур пропустил безопасные переходы `main.php?get_id=56&act=10&go=inv&wca=4` и `get_id=57...` для надевания `Дварфийский фонарь`; блокировка была не в `ProxyRequestQueue`, а в UI/серверном mine.js (`disable_move/time_left` держали кнопки недоступными).
- [x] Исправить существующий `MineJs.cs`, не создавая новый HTTP-контур: в текущий `ButtonPlace` добавлены независимые кнопки `Ваш персонаж` и `Инвентарь`, которые берут `vcode` из уже загруженных server menu arrays (`inf`/`inv`) и делают обычный `window.location` на `go=inf/go=inv` даже при активном `time_left/disable_move`. Авто-ход/авто-добыча продолжают ждать `NeverTimer` через прежний `routeWait()`/`autoWait()`.
- [x] По логам `ANClient/bin/Debug/Logs/Critical/20260522_12_20_*` подтверждено: ручные `Ваш персонаж`/`Инвентарь` срабатывают при `NeverTimer` (`manual menu open ... during timer=true`), а `Начать добычу` сейчас идёт только через авто-контур `__ancTryMineDig`, который ждёт `autoWait()` и зависит от `АвтоШахтёр`/готовой кирки. Исправление должно быть в существующем `MineJs.cs`: добавить ручную кнопку `Начать добычу`, вызывающую штатный `Digg(code)` напрямую, без нового HTTP-клиента; авто-добыча остаётся на прежнем guarded wait-контуре.
- [x] По логам `ANClient/bin/Debug/Logs/Critical/20260522_12_40_*` подтверждено: ручная `Начать добычу` отправляла `Digg(code)` при активном `NeverTimer` (`manual Digg click ... wait=471050/412157`), а сервер отвечал `Вы не можете сейчас начать добычу.`. Это не лечится подменой `vcode` из инвентаря: ошибка означает server cooldown. Исправление внесено в существующий `MineJs.cs`: ручной клик при `autoWait()>0` ставит один отложенный manual retry после таймера, не отправляя ранний `Digg`; `MineAjaxPhp`/`AutoMineRuntime` теперь логируют серверный отказ cooldown, если он всё же пришёл.
