...
- [x] Проверки после фикса хлама: изменённые файлы без BOM; в `AutoCutHandler.java`/`InventoryParser.java` не найдено mojibake, прямого `Log.*`, новых `AppVars.VCode` и старых AutoCut runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
