...
- [x] По live-логам `20260430_16_30`/`16_40` найден root cause фонового стопа после `route next: destination=12-248`: `AutoMoving` оставался true на `12-247`, `MainActivity.checkServerTimerDrivenActions()` каждые 2 сек грузил `go=inf&ab_nav_tick=1`, а свернутый WebView не выполнял JS `moveMapTo(...)`/`ButtonGen`, поэтому не было `DoHerbAutoCut` и `alchemy_ajax.php?act=1` до возврата UI.
- [x] Исправлена существующая ветка server-timer/MapAjax без отдельного scheduler: для AutoCut-owned `AutoMoving` в фоне `MainActivity` отправляет один native WebView `map_ajax.php?act=1` step из текущего `MapPath`/`ExtMap.MovableCells`, а `MapAjax.process(GO@...)` обновляет `MapLocation`, movable-cells и завершает `AutoMoving` при достижении destination.
- [x] После достижения AutoCut destination в фоне `MapAjax` планирует existing one-shot look retry, `MainActivity` сначала обновляет карту при stale `ContentMainPhp`, затем отправляет direct `alchemy_ajax.php?act=1` только когда HTML карты совпадает с текущей клеткой и содержит `look/ogl` vcode. Foreground JS-контур не меняется.
- [x] Для no-captcha среза в фоне `AlchemyAjaxPhp.submitAlchemyAct3ViaAjax(...)` получил fallback на `ACTION_WEBVIEW_LOAD_URL`, потому `evaluateJavascript(AjaxGet(...))` может не выполняться при свернутом WebView. В foreground остаётся прежний `AjaxGet(...)` flow.
- [x] Экран `Логи` исправлен в существующем `LogsActivity`: папки `Critical/`, `pool/`, `Logcat/` теперь открываются по клику, Back возвращает на уровень выше, файлы открываются/отправляются как раньше, отправка папки использует `ACTION_SEND_MULTIPLE`, а long tap включает закрепленную верхнюю панель иконок с multi-select.
- [ ] Проверить live-лог после фонового AutoCut-step: должны появиться `SERVER_TIMER_TICK background nav step`, `AutoMoving: destination reached ... via map_ajax GO`, затем `SERVER_TIMER_TICK background auto-cut direct look` и `act1: selected herb`/`act1: no selected available herb` без ручного разворота клиента.
- [ ] Проверить экран `Логи` на устройстве: открыть `Critical/`, `pool/`, `Logcat/`, long tap по файлу, отметить/снять несколько элементов, отправить один файл, несколько файлов и папку целиком через системный share sheet.
- [x] По логам `20260430_18_30` найден root cause первой Auto-Boi captcha: `MainActivity.updateCaptchaImageFromCaptured(...)` переключал fight popup с ожидаемой пары `captchaUrl=283260...` + `finishUrl/vcode=cc5e...` на свежеперехваченный, но уже чужой `captchaUrl=431497...`, сохраняя новый `finishUrl`. Anti-Captcha решила старую картинку и отправила `54508` с новым `vcode`, после чего сервер выдал повторную captcha.
- [x] Исправлена существующая ветка синхронизации captcha popup без нового scheduler/raw HTTP: переключение на `latest challenge` теперь разрешено только если перехваченный `code.php` совпадает с текущим `AppVars.CodeAddress` из parser-flow. Чужие свежие bytes логируются как `skip foreign latest challenge` и не подменяют активный popup.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после fight captcha guard — успешно.
- [x] Проверки после fight captcha guard: изменённые файлы без BOM; в `MainActivity.java` не найдено mojibake, прямого `Log.*`, новых `AppVars.VCode` и старых AutoCut runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` установлен на `192.168.1.100:5555` через `adb install -r` — успешно.
- [x] Сверена PC-реализация `ABClient/ExtMap/Map.cs`: при наличии `HerbCell.UpdatedInTicks` строка `Травы <group>` получает цвет свежести и время проверки клетки в формате `(HH:mm)` для свежего среза до 6 часов.
- [x] Исправлена существующая app2 UI-ветка `WebAppInterface.CellDivText(...)` без нового `Оглядеться`/HTTP-контура: время берётся из уже persisted snapshot-а `AutoCutManager.updateCurrentCellSnapshot(...)`, который обновляется после `HerbsList(...)`/`RESO@`.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после добавления времени проверки клетки на карту — успешно.
- [x] Проверки после timestamp-фикса: изменённые файлы без BOM; в изменённых Java-файлах не найдено mojibake, прямого `Log.*`, новых `AppVars.VCode` и старых AutoCut runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` после timestamp-фикса установлен на `192.168.1.100:5555` через `adb install -r` — успешно.
- [ ] Проверить live после ручного `Оглядеться`: на клетке с `HerbGroup` строка карты должна стать вида `Травы 11 (18:42)` рядом/после группы, цвет должен меняться по свежести как в PC-клиенте.
- [x] По свежим логам `20260430_20_40` найден root cause второй stale Auto-Бой captcha после завершения боя: `MainActivity.isActiveFightContext(...)` вызывал `new LezFight(html)` только для проверки активности, но `LezFight.ParseNonFight()` побочно записывал stale `AppVars.FightLink`/`AppVars.CodeAddress` из старого fight HTML; затем `AutoModeForegroundService.maybeShowFightCaptchaDialog(...)` показывал новый popup уже после `registerFightEnd`.
- [x] Исправлена существующая decision-point ветка без нового scheduler/raw HTTP: `isActiveFightContext(...)` теперь сохраняет и восстанавливает `AppVars.FightLink`/`AppVars.CodeAddress` после validation-парсинга, поэтому stale `code=????`/`code.php` не попадает в foreground-service как pending fight captcha.
- [ ] Проверить live после stale-captcha фикса: после `registerFightEnd` не должно быть нового `showCaptchaDialog` для старого `vcode`/`code.php`, Anti-Captcha не должна создавать второй task, а service не должен писать `fight captcha popup requested` без активного fight context.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после stale-captcha фикса — успешно.
- [x] Проверки после stale-captcha фикса: изменённые файлы без BOM; в `MainActivity.java` не найдено mojibake, прямого `Log.*` и новых `AppVars.VCode`; новые строки diff без старых AutoCut runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` после stale-captcha фикса установлен на `192.168.1.100:5555` через `adb install -r` — успешно.
- [x] По свежим логам `20260430_21_20` подтверждено: `Бесполезный хлам` был найден в `act=3` в `21:22:46`, cleanup ушёл через существующий inventory-flow, но `go=inv` во время server cooldown вернул HTML без rows; удаление сработало только после `NeverTimer` в `21:24:10`. После этого chat-масса оставалась stale (`1703/1615` -> `1708/1615`), потому snapshot массы обновился до drop и не инвалидировался после cleanup.
- [x] По жалобе на обычное отображение инвентаря найдено: `InventoryParser.mainPhpInv(...)` выбрасывал `Бесполезный хлам` только при `AutoCutCleanupPending`, а вне AutoCut-cleanup не подключал его к ветке просроченных предметов.
- [x] Исправлена существующая inventory-parser ветка без raw-drop контура: `Бесполезный хлам` теперь выбрасывается при любом полном отображении инвентаря через тот же `InvEntry.DropLink`, что и просрочка; cache-only sync не тронут. Лог `garbage bulk-drop redirect: link=..., cleanupPending=...` показывает, был ли это AutoCut-cleanup или обычный inventory pass.
- [x] Исправлена stale-масса после cleanup: `AutoCutManager.onCleanupCompleted(...)` инвалидирует `AppVars.AutoFishMassa`, чтобы следующий chat-report не продолжал прибавлять травы к pre-drop snapshot и не показывал ложный перевес.
- [x] По логам `20260430_21_30` найден root cause остановки/зависания после wrong captcha: `AlchemyAjaxPhp.processAlchemyAct1(...)` удалял due herb timer текущей клетки сразу на `act=1`; затем при wrong captcha `act=3` retry возвращался на карту, но `DoHerbAutoCut()` видел клетку уже checked/no due timer и писал только `current cell not ready`.
- [x] Исправлена существующая retry/timer ветка без нового scheduler: due herb timer удаляется только при no-selected scan или успешном `act=3`; пока есть `pendingCut`, `shouldAutoLookOnCurrentCell()` не запускает повторный `Оглядеться` параллельно captcha/act3.
- [ ] Проверить live: при ручном открытии инвентаря с `Бесполезный хлам` должен появиться `garbage bulk-drop redirect: link=..., cleanupPending=false`, предмет должен удалиться штатным `get_id=50`.
- [ ] Проверить live AutoCut wrong-captcha: после `act3: wrong protection/captcha code response` должен быть retry текущей клетки, а не бесконечное `DoHerbAutoCut=false, current cell not ready`.
- [x] По свежим логам `20260430_23_20` найден root cause ложной боевой captcha после submit в `23:21:06`: сервер вернул новый `fexp` с `vcode=3665...`, `LezFight.ParseNonFight()` корректно собрал normal `BuildFightLink(normal)` без `code=????`, но `FightAuto.resolveFightCaptchaUrl()` всё равно считал `fexp[4]` captcha-token обязательной капчей и открыл popup с finishUrl без `code=????`.
- [x] Исправлена существующая точка `FightAuto.extractCaptchaUrlFromFexp(...)` без нового captcha/scheduler контура: теперь fallback по `fexp[4]` используется только при том же признаке, что и в `LezFight.ParseNonFight()` (`fexp[6] == "0"`). Если `fexp[6]` не требует captcha, token логируется как ignored и finish-flow должен идти прямым `act=7` по normal link.
- [x] Убран незавершённый регрессивный redirect `main.php?wca=60` из cleanup `Бесполезный хлам`: рабочий путь остаётся через существующий `mainPhpFindInvWithFallback(..., "&im=0", ...)` с `go=inv&vcode`, а удаление выполняет `InventoryParser` по штатному `InvEntry.DropLink`.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после фикса `fexp[6]` captcha-guard — успешно.
- [x] Проверки после фикса: изменённые файлы без BOM; в diff не найдено mojibake-паттернов и старых AutoCut runtime-префиксов; прямой `Log.*` найден только в разрешённых `FileLogger`/`LogcatFileRecorder`; новых `AppVars.VCode` нет; `main.php?wca=60`/`AUTO_CUT_GARBAGE_INV` в Java нет; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` после фикса установлен на `192.168.1.100:5555` через `adb install -r` — успешно.
- [ ] Проверить live Auto-Бой после captcha submit: после `main.php?code=<digits>&get_id=61&act=7...` не должно быть повторных `get_id=61&act=7` с новыми `vcode`; результат должен перейти в обычный `main.php`/карту как в `BoiBot_captcha.har`.
- [x] По эталонному `BoiBot_captcha.har` проверено ручное завершение боя с капчей: браузер после финальной страницы с `code.php?...` отправляет один `GET main.php?code=<digits>&get_id=61&act=7&fexp=...&vcode=...` и получает обычную карту (`view_map()`), без повторных `act=7`.
- [x] По свежим логам `20260501_05_00` найдено отличие app2: после `showCaptchaDialog: submitting ...code=86526...act=7...` сервер вернул frame с `fexp[4]` token и `fexp[6]=30`, но Android считал это `BuildFightLink(normal)` и каждые 1-2 секунды заново отправлял `get_id=61&act=7` с новым `vcode`.
- [x] Исправлена существующая finish-flow ветка без нового HTTP/scheduler-контура: `LezFight.ParseNonFight()` теперь повторяет C#-семантику и не строит normal `FightLink`, если `fexp[4]` содержит captcha-token, но `fexp[6] != "0"`; fallback-и `FightAuto.extractFightFinishLinkFromHtml(...)` и `extractFightCleanVcodeFromFexp(...)` также не восстанавливают `act=7`/`act=5` из такого `fexp`.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после HAR/finish-loop фикса — успешно.
- [x] Проверки после HAR/finish-loop фикса: изменённые файлы без BOM; в изменённых Java-файлах нет mojibake, прямого `Log.*`, новых `AppVars.VCode` и новых старых runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` после HAR/finish-loop фикса установлен на `192.168.1.100:5555` через `adb install -r` — успешно.
- [x] По свежим логам `20260501_06_40` найден root cause ответа Anti-Captcha `12345`: в `06:46:35`/`06:46:40` в solver ушли изображения `code.php` размером `258` bytes, тогда как рабочие captcha из старых логов имеют несколько килобайт. Маленькая заглушка декодировалась как картинка, из-за чего popup считал challenge готовым и `AntiCaptchaManager` создавал task.
- [x] Исправлена существующая ветка `MainActivity.showCaptchaDialog(...) -> updateCaptchaImageFromCaptured(...) -> maybeStartAntiCaptchaForActiveChallenge(...)` без нового solver/scheduler-контура: добавлен guard `CAPTCHA_IMAGE_MIN_USABLE_BYTES=1024` и общий `decodeUsableCaptchaBitmap(...)`. Captcha bytes меньше порога или без валидного bitmap больше не lock-ятся, не публикуются как готовая картинка и не отправляются в Anti-Captcha.
- [ ] Проверить live после blank-captcha guard: при `code.php` около `258` bytes должны появиться логи `captcha image bytes too small`, но не должно быть `ANTI_CAPTCHA_TRACE create task, bytes=258`; нормальная captcha размером несколько килобайт должна по-прежнему уходить в Anti-Captcha.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после blank-captcha guard — успешно.
- [x] Проверки после blank-captcha guard: `MainActivity.java` и task-файл без BOM; в `MainActivity.java` нет mojibake, прямого `Log.*` и новых `AppVars.VCode`; в diff нет старых runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` после blank-captcha guard установлен на `192.168.1.100:5555` через `adb install -r` — успешно; `LoginActivity` стартует, процесс `ru.neverlands.anclient` запущен.
- [x] По свежим логам `20260501_08_00` восстановлена цепочка Авто-Травника: `DoHerbAutoCut=true`, `act=1`, выбранная трава `Пшеница`, `captchaRequired=true`, popup открыт, captcha image `4712` bytes успешно загружена, `ANTI_CAPTCHA_TRACE create task` был отправлен. Срез не произошёл не из-за отсутствия запроса, а из-за ответа сервиса `ERROR_NO_SLOT_AVAILABLE: No idle workers are available...`; после этого app2 логировал только `failed for active popup: createTask failed` и не делал retry.
- [x] Исправлена существующая ветка `AntiCaptchaManager -> MainActivity.onFailed(...)` без нового solver-контура: `AntiCaptchaManager` теперь пробрасывает конкретный `createTask failed: ERROR_NO_SLOT_AVAILABLE...`, а `MainActivity` распознаёт временные ошибки (`ERROR_NO_SLOT_AVAILABLE`, `no idle workers`, `HTTP 5xx`, `timeout`) и планирует ограниченный retry текущего popup через уже существующий `fightCaptchaHandler`.
- [ ] Проверить live после retry-fix: при `ERROR_NO_SLOT_AVAILABLE` должны быть логи `ANTI_CAPTCHA_TRACE retry scheduled` и затем `retry start`; при успешном повторе должен появиться `solved`, `showCaptchaDialog: submit source=anti_captcha` и `alchemy_ajax.php?act=3`.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после retry-fix — успешно.
- [x] Проверки после retry-fix: изменённые `MainActivity.java`, `AntiCaptchaManager.java` и task-файл без BOM; в изменённых Java-файлах нет mojibake и прямого `Log.*`; новых `AppVars.VCode` нет; diff без старых runtime-префиксов; `git diff --check` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK после retry-fix установлен на `192.168.1.100:5555` через `adb install -r` — успешно; `ru.neverlands.anclient` запущен, `versionName=1.1.5`, свежих `FATAL EXCEPTION` в коротком logcat-срезе нет.
- [x] По свежим логам `20260501_08_50`/`08:56` подтверждено, что `multi_cut` не зависал в сети: после успешного `act=3` сервер поставил реальный `NeverTimer=73000`, retry сработал только после `msLeft=0`, но `DoHerbAutoCut=false, current cell not ready` заблокировал повторный `Оглядеться`.
- [x] Root cause `08:56`: текущая клетка `12-341` оставалась в checked-set, а due herb timer был очищен фактическим `act=1`; поэтому `shouldAutoLookOnCurrentCell()` считал клетку уже проверенной и не запускал повторный scan для оставшейся выбранной травы.
- [x] Исправлена существующая ветка `AutoCutManager.markHerbCut(...)` без нового scheduler/HTTP-контура: при `retrySameCellAfterCut=true` текущая клетка снимается из checked-set через `unmarkCurrentCellChecked("multi_cut:" + source)` перед `scheduleLookRetryAfterTimer(...)`.
- [ ] Проверить live после multi-cut checked-set фикса: после успешного среза с оставшейся выбранной травой должны быть `cell unchecked for retry`, retry после `NeverTimer`, затем `DoHerbAutoCut=true`, `AUTO_CUT_JS start Ogl` и новый `alchemy_ajax.php?act=1` на той же клетке.
- [x] `./gradlew.bat --no-daemon :app2:assembleDebug` после multi-cut checked-set фикса — успешно.
- [x] Проверки после multi-cut checked-set фикса: изменённые файлы без BOM; в Java diff нет mojibake, прямого `Log.*`, новых `AppVars.VCode` и старых AutoCut runtime-префиксов; общая проверка `Log.*` показывает только разрешённые `AppLog.java`, `FileLogger.java`, `LogcatFileRecorder.java`; `git diff --check -- app2 TODO2` показал только известный шум `.gitattributes:7` и LF/CRLF warnings.
- [x] APK `app2/build/outputs/apk/debug/anclient_v1.1.5.apk` после multi-cut checked-set фикса установлен на `192.168.1.100:5555` через `adb install -r` — успешно; `ru.neverlands.anclient` запущен, `versionName=1.1.5`, в коротком logcat-срезе PID приложения нет `FATAL EXCEPTION`.
