...
public class ExtMap {
    ...
    private static final SimpleDateFormat[] ANC_VISITED_FORMATS = new SimpleDateFormat[] {
    ...
    private static final SimpleDateFormat ANC_VISITED_OUTPUT_FORMAT =
            new SimpleDateFormat("M/d/yyyy h:mm:ss a", Locale.US);
    private static final String ANC_VISITED_ZERO = "1/1/0001 12:00:00 AM";
    ...
    private static void loadAncMap(Context context) {
        AssetManager assets = context.getAssets();
        Map<String, Integer> ancCosts = new HashMap<>();
        Map<String, String> ancLabels = new HashMap<>();
        Map<String, String> ancRegions = new HashMap<>();
        ...
        try {
            ...
            if (context.getExternalFilesDir(null) != null) {
                externalFile = new File(context.getExternalFilesDir(null), "ancells.xml");
            }
            if (externalFile != null && externalFile.exists()) {
                input = new FileInputStream(externalFile);
                source = externalFile.getAbsolutePath();
            } else {
                input = assets.open("ancells.xml");
            }
        } catch (Exception openError) {
            AppLog.e(TAG, "loadAncMap source open error: " + openError.getMessage());
        }
            ...
            while (event != XmlPullParser.END_DOCUMENT) {
                if (event == XmlPullParser.START_TAG && "cell".equalsIgnoreCase(parser.getName())) {
                    ...
                    if (regnum != null) {
                        ...
                        ancLabels.put(regnum, label != null ? label : "");
                        String region = parser.getAttributeValue(null, "region");
                        String normalizedRegion = normalizeRegionLabel(region);
                        if (!normalizedRegion.isEmpty()) {
                            ancRegions.put(regnum, normalizedRegion);
                        }
                        String visited = parser.getAttributeValue(null, "visited");
                        long visitedMs = parseAncVisitedMs(visited);
                        if (visitedMs > 0L) {
                        ...
                        ancCosts.put(regnum, cost);
                    }
                }
                event = parser.next();
            }
            AppLog.d(TAG, "loadAncMap: source=" + source
                    + ", cells=" + ancCosts.size()
                    + ", visitedLoaded=" + loadedVisitedCount);
        } catch (Exception e) {
            AppLog.e(TAG, "loadAncMap error: " + e.getMessage());
        }

        for (String regnum : new java.util.ArrayList<>(Cells.keySet())) {
            if (!ancCosts.containsKey(regnum)) {
                Cells.remove(regnum);
            }
        }
        for (Map.Entry<String, Integer> e : ancCosts.entrySet()) {
            String regnum = e.getKey();
            int cost = e.getValue();
            if (Cells.containsKey(regnum)) {
                Cells.get(regnum).Cost = cost;
                if (ancLabels.containsKey(regnum)) {
                    Cells.get(regnum).Name = ancLabels.get(regnum);
                }
                String region = ancRegions.get(regnum);
                ...
                cell.Cost = cost;
                cell.Name = ancLabels.containsKey(regnum) ? ancLabels.get(regnum) : "";
                cell.Region = normalizeRegionLabel(ancRegions.get(regnum));
    ...
    private static long parseAncVisitedMs(String rawValue) {
        if (rawValue == null) {
            return 0L;
        }
        ...
        for (SimpleDateFormat format : ANC_VISITED_FORMATS) {
            try {
    ...
    private static boolean persistVisitedToExternalFile(Context context) {
        ...
        File targetFile = new File(externalDir, "ancells.xml");
        if (!targetFile.exists()) {
            try {
                copyAssetToFile(context, "ancells.xml", targetFile);
            } catch (Exception e) {
                AppLog.e(TAG, "persistVisited: cannot create target ancells.xml", e);
                return false;
            }
        }
        File tmpFile = new File(externalDir, "ancells.xml.tmp");
            ...
            while (event != XmlPullParser.END_DOCUMENT) {
                if (event == XmlPullParser.START_TAG) {
                    ...
                    for (int i = 0; i < attrCount; i++) {
                        ...
                        if ("visited".equalsIgnoreCase(attrName)) {
                            ...
                            if (visitedMs != null && visitedMs > 0L) {
                                attrValue = formatVisitedForAnc(visitedMs);
                            }
                    ...
                    if ("cell".equalsIgnoreCase(tag) && !hasVisitedAttr && regnum != null) {
                        ...
                        if (visitedMs != null && visitedMs > 0L) {
                            serializer.attribute(null, "visited", formatVisitedForAnc(visitedMs));
                        }
                    }
        ...
        try {
            if (targetFile.exists() && !targetFile.delete()) {
                AppLog.w(TAG, "persistVisited: cannot delete old ancells.xml");
                return false;
            }
    ...
    private static String formatVisitedForAnc(long visitedMs) {
        if (visitedMs <= 0L) {
            return ANC_VISITED_ZERO;
        }
        synchronized (ANC_VISITED_OUTPUT_FORMAT) {
            return ANC_VISITED_OUTPUT_FORMAT.format(new Date(visitedMs));
        }
    }
    ...
    private static void loadTeleports(Context context) {
        AssetManager assets = context.getAssets();
        try (InputStream in = assets.open("anteleports.xml")) {
            XmlPullParserFactory factory = XmlPullParserFactory.newInstance();
            factory.setNamespaceAware(false);
            XmlPullParser parser = factory.newPullParser();
            parser.setInput(in, "UTF-8");
