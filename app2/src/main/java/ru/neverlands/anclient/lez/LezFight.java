package ru.neverlands.anclient.lez;
...
import ru.neverlands.anclient.model.*;
...
public class LezFight {
    ...
    private void BuildFrame() {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=windows-1251\"><title>ANClient</title></head><body><!--ANCLIENT_GENERATED-->");
        ...
        sb.append("var __anSubmitted=false;");
        sb.append("function __anSubmit(reason){");
        sb.append(" if(__anSubmitted){return;}");
        sb.append(" __anSubmitted=true;");
        sb.append(" try{");
        sb.append("  console.log('ANCLIENT_AUTOBATTLE_SUBMIT'+(reason?('_'+reason):''));");
        sb.append("  if(document&&document.ff&&typeof document.ff.submit==='function'){document.ff.submit();}");
        sb.append(" }catch(e){");
        sb.append("  console.log('ANCLIENT_AUTOBATTLE_SUBMIT_ERR:'+e);");
        sb.append(" }");
        sb.append("}");
        sb.append("setTimeout(function(){ __anSubmit('timer'); }, ").append(delay).append(");");
        sb.append("window.addEventListener('beforeunload', function(){ __anSubmit('beforeunload'); });");
        sb.append("window.addEventListener('pagehide', function(){ __anSubmit('pagehide'); });");
        sb.append("document.addEventListener('visibilitychange', function(){ if(document.visibilityState==='hidden'){ __anSubmit('hidden'); } });");
        sb.append("document.addEventListener('click', function(e){");
        sb.append(" try{");
        sb.append("  var t=e&&e.target?e.target:null;");
        sb.append("  while(t){");
        sb.append("   if(t.tagName==='A'&&t.href){ __anSubmit('click'); break; }");
        ...
        try {
            ...
            switch (state) {
                ...
                case "3": {
                    ...
                    if (vcode.length() > 2) {
                        ...
                        AppVars.FightLink = "main.php?get_id=61&act=6&mode=" + mode
                                ...
                                + "&vcode=" + vcode;
                        if (vcode.length() >= 16) {
                            ru.neverlands.anclient.utils.SessionManager.getInstance().cacheFightVCode(vcode, "fightty_case3");
                        }
                    } else {
                ...
                case "5": {
                    ...
                    if (vcode.length() >= 16) {
                        ru.neverlands.anclient.utils.SessionManager.getInstance().cacheFightVCode(vcode, "fightty_case5");
                    }
                    break;
                }
                case "7": {
                    ...
                    if (vcode.length() >= 16) {
                        ru.neverlands.anclient.utils.SessionManager.getInstance().cacheFightVCode(vcode, "fightty_case7");
                    }
                    break;
                }
                default:
    ...
    private void BuildFightLink(boolean withCaptchaPlaceholder) {
        ...
        try {
            ...
            if (fexp3.length() >= 16) {
                ru.neverlands.anclient.utils.SessionManager.getInstance().cacheFightVCode(fexp3, "fexp_buildfightlink");
            }
