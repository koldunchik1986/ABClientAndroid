...
public class HpmpJs {
    ...
    public static byte[] process() {
        ...
        sb.append("try{");
        sb.append("var __ab_curTire = NaN;");
        sb.append("var __ab_maxTire = NaN;");
        sb.append("if (typeof hpmp !== 'undefined' && hpmp && hpmp.length > 4) { __ab_maxTire = parseInt(hpmp[4], 10); }");
        sb.append("if (!isNaN(__ab_maxTire)) { __ab_curTire = 100 - __ab_maxTire; }");
        sb.append("if (isNaN(__ab_curTire)) {");
        sb.append("var __ab_bridge = (window.AndroidBridge&&typeof window.AndroidBridge.GetCurrentTied==='function')?window.AndroidBridge:((window.external&&typeof window.external.GetCurrentTied==='function')?window.external:null);");
        sb.append("if (__ab_bridge) {");
        sb.append("var __ab_bridgeTied = parseInt(__ab_bridge.GetCurrentTied(), 10);");
        sb.append("if (!isNaN(__ab_bridgeTied)) { __ab_curTire = __ab_bridgeTied; }");
        sb.append("}");
        sb.append("}");
        sb.append("if (!isNaN(__ab_curTire)) {");
