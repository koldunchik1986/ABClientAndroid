...
public final class HtmlUtils {
    ...
    public static String getJsFix() {
        return "try { if (window.AndroidBridge) { window.external = window.AndroidBridge; } } catch(e) {}" +
                ...
                "if (typeof window.__abEnsureNode !== 'function') { window.__abEnsureNode = function(id, tag) { try { var list = document.querySelectorAll('[id=\\\"' + id + '\\\"]'); if (list && list.length) { for (var i = 0; i < list.length; i++) { if (!list[i].getAttribute || list[i].getAttribute('data-ab-stub') !== '1') return list[i]; } return list[0]; } var host = document.body || document.documentElement; if (!host) return { innerHTML: '', value: '', style: {} }; var el = document.createElement(tag || 'div'); el.id = id; el.setAttribute('data-ab-stub', '1'); el.style.display = 'none'; el.style.visibility = 'hidden'; host.appendChild(el); return el; } catch(e) { return { innerHTML: '', value: '', style: {} }; } }; }" +
                "if (!window.__abEnsureCriticalNodes) { window.__abEnsureCriticalNodes = function() { try { window.__abEnsureNode('transfer', 'div'); window.__abEnsureNode('complect', 'div'); window.__abEnsureNode('hbar', 'span'); } catch(e) {} }; }" +
                "if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', window.__abEnsureCriticalNodes); } else { window.__abEnsureCriticalNodes(); }" +
                "if (!document._abclientOrigGetElementById) { document._abclientOrigGetElementById = document.getElementById.bind(document); document.getElementById = function(id) { if (!id) return null; try { var list = document.querySelectorAll('[id=\\\"' + id + '\\\"]'); if (list && list.length) { for (var i = 0; i < list.length; i++) { if (!list[i].getAttribute || list[i].getAttribute('data-ab-stub') !== '1') return list[i]; } return list[0]; } } catch(e) {} var el = document._abclientOrigGetElementById(id); if (el) return el; try { var host = document.body || document.documentElement; if (!host) return null; var dummy = document.createElement('span'); dummy.id = id; dummy.setAttribute('data-ab-stub', '1'); dummy.style.display = 'none'; host.appendChild(dummy); return dummy; } catch(e2) { return null; } }; }" +
