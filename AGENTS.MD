...
0.  **Кодировка файлов (UTF-8) и анти-mojibake.**
    *   **Правило:** Все текстовые файлы проекта (`.java`, `.kt`, `.xml`, `.md`, `.gradle`, `.properties`, `.json`, `.js`, `.cs` и т.д.) сохранять в **UTF-8 (без BOM)**.
    *   **Запрещено:** Сохранять/перекодировывать файлы в `Windows-1251`/`CP1251` или любой другой legacy-кодировке.
    *   **Проверка:** Перед сдачей изменений проверять, что в диффе/файлах нет «кракозябр» вида `РЎР...`, `РџС...`, `Ð...`, `Ñ...`, `???` вместо кириллицы.
    *   **При правках кириллицы:** Если видны признаки mojibake, сначала восстановить корректный UTF-8 текст и только потом вносить функциональные изменения.
...
3.  **Безопасность `User-Agent` (анти-детект).**
    *   **Описание:** Сервер не должен видеть признаков неофициального клиента в HTTP-заголовках.
    *   **Правило:** В `User-Agent` **запрещено** использовать строки с упоминанием клиента/приложения (например: `ABClient`, `Android; ABClient`, любые кастомные метки).
    *   **Требование:** Для всех HTTP-запросов использовать только браузерный `User-Agent` (в стиле обычного Chrome/браузера), согласованный по проекту.
    *   **Проверка перед сдачей:** Выполнить поиск по коду на `User-Agent`, `ABClient`, `Android;` и убедиться, что в сетевых заголовках нет идентификаторов клиента.
